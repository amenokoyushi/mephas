...
server <- shinyServer(
  ...
  output$bas2 <-
    renderTable({
      ## don't use renerPrint to do renderTable
      x <- Y()
      res <- stat.desc(x)[1:3,]
  ...
  output$bp2 = renderPlot({
    ...
    ggplot(mx, aes(x = variable, y = value, fill = variable)) +
      geom_boxplot(width = 0.4,outlier.colour = "red",alpha = .3) +
      geom_jitter(width = 0.1, size = 1.5) + ylab(" ") + xlab(" ") + ggtitle("") +
  ...
  output$meanp2 = renderPlot({
    ...
    p1 = ggplot(des, aes(x = rownames(des), y = mean, fill = rownames(des))) +
      geom_errorbar(width = .1, aes(ymin = mean - std.dev, ymax = mean + std.dev), data = des) +
      xlab("") + ylab(expression(Mean %+-% SD)) + geom_point(shape = 21, size =3) +
      theme_minimal() + theme(legend.title = element_blank())
    p2 = ggplot(des, aes(
      x = rownames(des),
      y = mean,
      fill = rownames(des)
    )) + xlab("") + ylab(expression(Mean %+-% SD)) +  geom_bar(
      ...
      geom_errorbar(
        ...
        aes(ymin = mean - std.dev, ymax = mean + std.dev),
        data = des
      ) + theme_minimal() + theme(legend.title = element_blank())
  ...
  output$makeplot2 <- renderPlot({
    ...
    plot3 <-
      ggplot(mx, aes(
        ...
        colour = variable,
        fill = variable
      )) + geom_histogram(binwidth = input$bin2,
                          alpha = .3,
                          position = "identity")  + ggtitle("Histogram") + xlab("") + theme_minimal() + theme(legend.title =
                                                                                                                element_blank())
    plot4 <-
      ggplot(mx, aes(x = value, colour = variable)) + geom_density() + ggtitle("Density Plot") + xlab("") + theme_minimal() + theme(legend.title =
                                                                                                                                      element_blank())
  ...
  output$var.test <- renderTable({
    ...
    res.table <- t(
      data.frame(
        ...
        P_value = res$p.value,
        Confidence_interval_0.95 = paste0(
          "(",
  ...
  output$t.test2 <- renderTable({
    ...
    res1.table <- t(
      data.frame(
        ...
        Confidence_interval_0.95 = paste0(
          "(",
          round(res1$conf.int[1], digits = 4),
          ", ",
          round(res1$conf.int[2], digits = 4),
          ")"
        ),
  ...
  output$meanp.p = renderPlot({
    ...
    p1 = ggplot(des, aes(
      ...
      xlab("") + ylab(expression(Mean %+-% SD)) + geom_point(shape = 21, size =
                                                               3) + theme_minimal() + theme(legend.title = element_blank())
    p2 = ggplot(des, aes(
      x = rownames(des),
      y = mean,
      fill = rownames(des)
  ...
  output$makeplot.p <- renderPlot({
    ...
    plot2 <-
      ggplot(x, aes(x = x[, 3])) + geom_histogram(
        colour = "black",
        fill = "grey",
        binwidth = input$bin.p,
        position = "identity"
      ) + xlab("") + ggtitle("Histogram") + theme_minimal() + theme(legend.title =
                                                                      element_blank())
    plot3 <-
      ggplot(x, aes(x = x[, 3])) + geom_density() + ggtitle("Density Plot") + xlab("") + theme_minimal() + theme(legend.title =
                                                                                                                   element_blank())
  ...
  output$meanp = renderPlot({
    x <- as.numeric(unlist(strsplit(input$x, "[\n, \t, ]")))
    des = data.frame(t(stat.desc(x)))
    p1 = ggplot(des, aes(x = rownames(des), y = mean)) + geom_errorbar(
      width = .1,
      aes(ymin = mean - std.dev, ymax = mean + std.dev),
      data = des
    ) +
      xlab("") + ylab(expression(Mean %+-% SD)) + geom_point(shape = 21, size =
                                                               3) + theme_minimal() + theme(legend.title = element_blank())
    p2 = ggplot(des, aes(x = rownames(des), y = mean)) + xlab("") + ylab(expression(Mean %+-% SD)) +  geom_bar(
      position = position_dodge(),
      stat = "identity",
      width = 0.2,
      alpha = .3
  ...
  output$makeplot <- renderPlot({
    x <- Z()
    plot1 <-
      ggplot(x, aes(sample = x[, 1])) + stat_qq() + ggtitle("Normal Q-Q Plot") + xlab("") + theme_minimal()  ## add line,
    plot2 <-
      ggplot(x, aes(x = x[, 1])) + geom_histogram(
        colour = "black",
        fill = "grey",
        binwidth = input$bin,
        position = "identity"
                                                                      ...
                                                                      element_blank())
    plot3 <-
      ggplot(x, aes(x = x[, 1])) + geom_density() + ggtitle("Density Plot") + xlab("") + theme_minimal() + theme(legend.title =
                                                                                                                   element_blank())
    grid.arrange(plot1, plot2, plot3, ncol = 3)
  })
